...
class SigWebadmin extends StatefulWidget {
  const SigWebadmin({super.key, required this.title});

  final String title;

  @override
  State<SigWebadmin> createState() => _SigWebState();
}

class _SigWebState extends State<SigWebadmin> {
 ...
 void _onMapCreated(GoogleMapController controller) {
  ...
  Future<void> uploadGeoJsonToStorage(
    ...
    try {
      final storageRef =
          FirebaseStorage.instance.ref().child('donnees_geojson_files/$fileName');
  ...
  Future<void> uploadGeoJsonToFirestore(
    ...
    try {
      ...
      await firestore.collection('donnees_geojson_files').doc(documentId).set({
        'geojson': geoJsonData,
      });
  ...
  Future<void> _showGeoJsonSelectionDialog2() async {
    try {
      final storage = FirebaseStorage.instance;
      final listResult = await storage.ref('donnees_geojson_files').listAll();
      print(
  ...
  Future<void> _uploadGeoJsonFile2() async {
    try {
      setState(() {
        uploadingData = true;
        uploadProgress = 0.0;
      ...
      appBar: AppBar(
        ...
        actions: [
          ...
          IconButton(
            ...
            onPressed: _uploadGeoJsonFile2,
            tooltip: 'télécharger des données',
          ),
          IconButton(
